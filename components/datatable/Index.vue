  ...
  <base-card>
    ...
    <v-card class="pa-2" v-show="modal">
      ...
      <v-data-table multi-sort :headers="headersSelected" :items="items" :items-per-page="itemPerPageForTable"
        class="elevation-1 table-one row-pointer" :loading="items.length <= 0"
        loading-text="Chargement... Veuillez patienter" :search="search" @click:row="handleClickRow"
        :sort-desc="sortDescForTable" :sort-by="sortByForTable" item-key="id" :footer-props="{
    ...
    <v-card class="pa-10 text-center" v-show="!modal">
      ...
      <v-table class="text-center" justify="space-between">
        ...
        <tr>
          ...
          <td>
            <v-btn v-if="!toggleDiag.date_depart_retour && toggleDiag.date_arrivee_aller"
              @click="onButtonClick(toggleDiag)">départ retour</v-btn><br />
          </td>
        </tr>
        <tr>
          <td class="pa-8">
          ...
          <td>
            <v-btn v-if="!toggleDiag.date_arrivee_retour && toggleDiag.date_depart_retour"
              @click="onButtonClick(toggleDiag)">arrivé retour</v-btn><br />
          </td>
        </tr>
      </v-table>
    </v-card>
...
export default {
  ...
  methods: {
    ...
    callDataAPI() {//permet de recupérer la base de données transport
